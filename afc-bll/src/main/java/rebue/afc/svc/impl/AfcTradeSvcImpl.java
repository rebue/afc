...
public class AfcTradeSvcImpl extends MybatisBaseSvcImpl<AfcTradeMo, java.lang.Long, AfcTradeMapper>
	...
	public void addTrade(final AfcTradeMo tradeMo) {
		...
		// if (newAccountMo.getCashbacking() != null &&
		// !newAccountMo.getCashbacking().equals(oldAccountMo.getCashbacking())) {
		...
		// if (newAccountMo.getCommissioning() != null &&
		// !newAccountMo.getCommissioning().equals(oldAccountMo.getCommissioning())) {
		...
		if (newAccountMo.getCommissionTotal() != null
				&& !newAccountMo.getCommissionTotal().equals(oldAccountMo.getCommissionTotal())) {
		...
		if (newAccountMo.getDepositUsed() != null
				&& !newAccountMo.getDepositUsed().equals(oldAccountMo.getDepositUsed())) {
		...
		if (newAccountMo.getWithdrawing() != null
				&& !newAccountMo.getWithdrawing().equals(oldAccountMo.getWithdrawing())) {
	 ...
	 */
	@Override
	public PageInfo<AfcTradeListRo> personTradeList(AfcTradeTo to, int pageNum, int pageSize) {
		_log.info("list: to-{}; pageNum-{}; pageSize-{}", to, pageNum, pageSize);
		Byte[] tradeType = to.getTradeType();
		...
		if (to.getAccountName() != null) {
			String accountIdStr = "(";
			List<SucUserMo> userMoResultList = sucUserSvc.getByWxNick(to.getAccountName());
		...
		for (int i = 0; i < tradeList.size(); i++) {
			...
			SucUserMo sucMo = sucUserSvc.getById(ro.getAccountId());
			if (sucMo != null) {
	 ...
	 */
	@Override
	public PageInfo<AfcTradeListRo> orgTradeList(AfcTradeTo to, int pageNum, int pageSize) {
		_log.info("list: to-{}; pageNum-{}; pageSize-{}", to, pageNum, pageSize);
		Byte[] tradeType = to.getTradeType();
		if (tradeType != null && tradeType.length > 0) {
			String tradeTypeString = "(";
		...
		if (to.getAccountName() != null) {
			String accountIdStr = "(";
			SucOrgMo sucOrgMo = sucOrgSvc.getOne(to.getAccountName());
