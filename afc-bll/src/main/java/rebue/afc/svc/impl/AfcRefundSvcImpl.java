...
@Service
public class AfcRefundSvcImpl extends MybatisBaseSvcImpl<AfcRefundMo, java.lang.Long, AfcRefundMapper>
		implements AfcRefundSvc {
	...
	public Ro refund(final RefundTo to) {
		...
		if (to.getBuyerAccountId() == null //
				|| (to.getRefundAmount() == null && to.getReturnBalanceToBuyer() == null
						&& to.getReturnCashbackToBuyer() == null) //
				|| to.getOpId() == null//
		...
		_log.debug("计算支付总额，支付V支付的返现金总额，支付V支付的余额总额：orderId-{}, buyAccountId-{}", to.getOrderId(),
				to.getBuyerAccountId());
		...
		for (final AfcPayMo pay : pays) {
			...
			if (pay.getPayTypeId() == PayTypeDic.VPAY.getCode()) {
				BigDecimal payAmount1 = pay.getPayAmount1();
				BigDecimal payAmount2 = pay.getPayAmount2();
				if (payAmount1 == null)
		...
		_log.debug("计算已退款总额: 已退款总额-{}，已退款V支付的返现金总额-{}，已退款V支付的余额总额-{}", paidTotal, refundedCashbackTotal,
				refundedBalanceTotal);
