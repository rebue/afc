...
public class AfcWithdrawAccountSvcImpl extends MybatisBaseSvcImpl<AfcWithdrawAccountMo, java.lang.Long, AfcWithdrawAccountMapper> implements AfcWithdrawAccountSvc {
    ...
    public int add(final AfcWithdrawAccountMo mo) {
        _log.info("添加提现账户信息");
        // 如果id为空那么自动生成分布式id
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public int modify(final AfcWithdrawAccountMo mo) {
        // 修改时间戳
        mo.setModifiedTimestamp(System.currentTimeMillis());
    ...
    public Boolean existByUserId(final Long userId) {
        final AfcWithdrawAccountMo mo = new AfcWithdrawAccountMo();
     ...
     */
    @Override
    public List<AfcWithdrawAccountMo> listByUserId(final Long userId) {
        final AfcWithdrawAccountMo mo = new AfcWithdrawAccountMo();
        mo.setAccountId(userId);
        return list(mo);
    }
     ...
     */
    @Override
    public AfcWithdrawAccountInfoRo getWithdrawAccountInfo(final Long userId) {
        _log.info("根据用户id查询用户提现账户信息的参数为：{}", userId);
        AfcWithdrawAccountInfoRo withdrawAccountInfoRo = new AfcWithdrawAccountInfoRo();
        final AfcWithdrawAccountMo withdrawAccountMo = new AfcWithdrawAccountMo();
        _log.info("根据用户id查询用户提现账户信息查询用户提现账户信息的参数为：{}", userId);
        withdrawAccountMo.setAccountId(userId);
        final AfcWithdrawAccountMo afcWithdrawAccountMo = afcWithdrawAccountSvc.getOne(withdrawAccountMo);
        ...
        final AfcAccountMo afcAccountMo = afcAccountSvc.getById(userId);
        ...
        final WithdrawNumberForMonthRo withdrawNumberForMonth = afcWithdrawSvc.getWithdrawNumberForMonth(afcWithdrawMo);
    ...
    @Override
    public PageInfo<AfcWithdrawAccountRo> listEx(final AfcWithdrawAccountMo mo, final int pageNum, final int pageSize, final String orderBy) {
        PageInfo<AfcWithdrawAccountRo> doSelectPageInfoEx = new PageInfo<>();
        final List<AfcWithdrawAccountRo> listEx = new ArrayList<>();
        final PageInfo<AfcWithdrawAccountMo> doSelectPageInfo = PageHelper.startPage(pageNum, pageSize, orderBy).doSelectPageInfo(() -> _mapper.selectSelective(mo));
        for (final AfcWithdrawAccountMo afcWithdrawAccountMo : doSelectPageInfo.getList()) {
            final AfcWithdrawAccountRo afcWithdrawAccountRo = dozerMapper.map(afcWithdrawAccountMo, AfcWithdrawAccountRo.class);
            _log.info("查询申请提现账号记录查询用户信息的参数为：{}", afcWithdrawAccountMo.getAccountId());
            final SucUserMo sucUserMo = sucUserSvc.getById(afcWithdrawAccountMo.getAccountId());
