...
public class AfcWithdrawSvcImpl extends MybatisBaseSvcImpl<AfcWithdrawMo, java.lang.Long, AfcWithdrawMapper> implements AfcWithdrawSvc {
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public WithdrawApplyRo apply(final ApplyWithdrawTo to) {
        _log.info("申请提现的参数为：{}", to);
        final WithdrawApplyRo ro = new WithdrawApplyRo();
        if (StringUtils.isAnyBlank(to.getBankAccountName(), to.getBankAccountNo(), to.getContactTel(), to.getIp()) || to.getApplicantId() == null || to.getWithdrawAmount() == null
                || to.getWithdrawType() == null) {
        ...
        final WithdrawNumberForMonthRo withdrawNumberForMonthRo = afcWithdrawSvc.getWithdrawNumberForMonth(afcWithdrawMo);
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public WithdrawOkRo ok(final WithdrawOkTo to) {
        // 当前时间
        final Date now = new Date();
        final WithdrawOkRo ro = new WithdrawOkRo();
        if (to.getId() == null || StringUtils.isAnyBlank(to.getMac(), to.getIp())) {
        ...
        final AfcAccountMo accountMo = accountSvc.getById(to.getAccountId());
    ...
    public WithdrawNumberForMonthRo getWithdrawNumberForMonth(final AfcWithdrawMo mo) {
        ...
        final double pow = Math.pow(2, withdrawCount);
    ...
    public PageInfo<AfcWithdrawRo> lisrEx(final AfcWithdrawMo mo, final int pageNum, final int pageSize, final String orderBy) {
        PageInfo<AfcWithdrawRo> doSelectPageInfoEx = new PageInfo<>();
        final List<AfcWithdrawRo> listEx = new ArrayList<>();
        final PageInfo<AfcWithdrawMo> doSelectPageInfo = PageHelper.startPage(pageNum, pageSize, orderBy).doSelectPageInfo(() -> _mapper.selectSelective(mo));
     ...
     */
    @Override
    public PageInfo<AfcWithdrawMo> selectWithdrawApplying(final Long accountId, final int pageNum, final int pageSize, final String orderBy) {
