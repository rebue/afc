...
@Service
public class AfcWithdrawAccountBindFlowSvcImpl extends MybatisBaseSvcImpl<AfcWithdrawAccountBindFlowMo, java.lang.Long, AfcWithdrawAccountBindFlowMapper> implements AfcWithdrawAccountBindFlowSvc {
    ...
    public int add(AfcWithdrawAccountBindFlowMo mo) {
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public Ro addEx(AfcWithdrawAccountBindFlowMo mo) {
    	...
    	_log.info("添加提现账户绑定流程的请求参数为：{}", mo);
    	int addResult = afcWithdrawAccountBindFlowSvc.add(mo);
     ...
     */
    @Override
    public AfcWithdrawAccountBindFlowMo getNewOneByApplicantId(Long applicantId) {
	...
	@Override
    public PageInfo<AfcWithdrawAccountBindFlowRo> listEx(AfcWithdrawAccountBindFlowMo mo, int pageNum, int pageSize, String orderBy) {
    	PageInfo<AfcWithdrawAccountBindFlowRo> doSelectPageInfoEx = new PageInfo<AfcWithdrawAccountBindFlowRo>();
    	List<AfcWithdrawAccountBindFlowRo> listEx = new ArrayList<AfcWithdrawAccountBindFlowRo>();
    	PageInfo<AfcWithdrawAccountBindFlowMo> doSelectPageInfo = PageHelper.startPage(pageNum, pageSize, orderBy).doSelectPageInfo(() -> _mapper.selectSelective(mo));
    	for (AfcWithdrawAccountBindFlowMo afcWithdrawAccountBindFlowMo : doSelectPageInfo.getList()) {
    		AfcWithdrawAccountBindFlowRo withdrawAccountBindFlowRo = dozerMapper.map(afcWithdrawAccountBindFlowMo, AfcWithdrawAccountBindFlowRo.class);
    		_log.info("查询申请提现账号记录查询用户信息的参数为：{}", afcWithdrawAccountBindFlowMo.getApplicantId());
    		SucUserMo sucUserMo = sucUserSvc.getById(afcWithdrawAccountBindFlowMo.getApplicantId());
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public Ro review(AfcWithdrawAccountBindFlowMo mo) {
    	_log.info("申请提现账号审核通过的参数为：{}", mo);
    	Ro ro = new Ro();
    	_log.info("申请提现账号审核通过查询申请提现账号信息的参数为：{}", mo.getId());
    	AfcWithdrawAccountBindFlowMo withdrawAccountBindFlowMo = _mapper.selectByPrimaryKey(mo.getId());
    	...
    	// 修改申请提现账号信息
    	int updateByPrimaryKeySelectiveResuly = _mapper.updateByPrimaryKeySelective(mo);
    	...
    	// 添加提现账号信息
    	AfcWithdrawAccountMo accountMo = new AfcWithdrawAccountMo();
    	...
    	int addResult = afcWithdrawAccountSvc.add(accountMo);
    ...
    @Override
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public int reject(AfcWithdrawAccountBindFlowMo mo) {
