...
public enum PrechargeResultDic implements EnumBase {
    /**
     * 1: 成功
     */
     ...
     */
    @JsonCreator
    public static PrechargeResultDic getItem(final int code) {
        final EnumBase result = valueMap.get(code);
        if (result == null) {
            throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
        }
        return (PrechargeResultDic) result;
    }
     ...
     * 构造器，传入code
     */
    PrechargeResultDic(final int code) {
        this.code = code;
    }
