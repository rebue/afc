...
public enum WithdrawDealResultDic implements EnumBase {
    /**
     * 1: 处理提交成功
     */
     ...
     * -1: 没有此处理人
     */
    @ApiModelProperty(value = "没有此处理人")
    NOT_FOUND_OP(-1),
    /**
     * -2: 处理人被锁定
     */
    @ApiModelProperty(value = "处理人被锁定")
    OP_LOCKED(-2),
    /**
     * -3: 没有此申请
     */
     ...
     */
    @JsonCreator
    public static WithdrawDealResultDic getItem(final int code) {
        final EnumBase result = valueMap.get(code);
        if (result == null) {
            throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
        }
        return (WithdrawDealResultDic) result;
    }
     ...
     * 构造器，传入code
     */
    WithdrawDealResultDic(final int code) {
        this.code = code;
    }
